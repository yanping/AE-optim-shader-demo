...
> 📖 **使用说明与快速上手**：
> - 完整操作指引与环境设置请参阅 [README_CN.md](README_CN.md)（英文版请见 [README.md](README.md)）。
...
```text
shader-optim-test/
├── PLAN.md                     # 📋 英文实施规划方案文档 (English)
├── PLAN_CN.md                  # 📋 简体中文实施规划方案文档 (简体中文)
...
├── README.md                   # 📖 用户使用指南与架构说明文档 (English)
├── README_CN.md                # 📖 用户使用指南与架构说明文档 (简体中文)
├── venv/                       # 🐍 Python 3.11 独立虚拟环境
├── alpha_evolve/               # 🔒 Google Cloud 官方 AlphaEvolve 客户端库 (严禁修改)
├── tests/                      # 🧪 自动化测试套件 (包含 72 项单元测试，100% 通过)
...
4. **技术改进点摘要**：提炼关键优化手法（如步长裁剪、超越函数近似、无分化展开等）。
...
| `make test` | 无 | 运行自动化单元测试套件（覆盖管线、评估器、画质、瓶颈、报告各模块，共 72 项测试） | `make test` |
| `make mask` | 无 | **项目交付脱敏**：一键将全项目配置文件、文档中的个人 `project_id`、`ge_app_id` 脱敏遮盖为占位符 | `make mask` |
| `make mask-dry` | 无 | **脱敏预览**：预览脱敏匹配的文件与出现频次（Dry-run），不实际修改文件 | `make mask-dry` |
...
| **Phase 5** | 报告增强与全量测试 | HTML 报告集成多通道 Breakdown 卡片；72 项自动化测试 100% 通过；实机云端闭环验证。 | `src/report.py`, `tests/test_pipeline.py` |
